...
function formatiereDatum(apiDatum) {
  ...
  async function fetchUser() {
    const response = await fetch('https://linkup-api.de/api/users/4');
  ...
  async function fetchPosts() {
    const response = await fetch('https://linkup-api.de/api/posts/user/4?limit=0&page=0');
    const data = await response.json();
    posts = data;
  }
...
<div class = "con" style="display: flex; flex-direction: row;">
    ...
    <div class=" bg-secondary-400 card p-4 max-h-[220px] overflow-auto space-y-4" style="border: 2px solid black; border-radius: 10px;">
